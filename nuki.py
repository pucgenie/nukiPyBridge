        ...
        if self.device == None:
            ...
            while (nukiBleConnectionReady == False and currentTries < retries):
                ...
                try:
                    ...
                    currentTries += 1
                    print
                    "Unable to connect, retrying..., retry count: " + currentTries
        print
...
def isNewNukiStateAvailable(self):
    ...
    newStateAvailable = -1
    print
    "isNewNukiStateAvailable() -> search through %d received beacons..." % len(returnedList)
    for beacon in returnedList:
        ...
        if beaconElements[0] == self.macAddress.lower() and beaconElements[1] == "a92ee200550111e4916c0800200c9a66":
            print
            "Nuki beacon found, new state element: %s" % beaconElements[4]
            ...
            print
            "non-Nuki beacon found: mac=%s, signature=%s" % (beaconElements[0], beaconElements[1])
    print
    "isNewNukiStateAvailable() -> result=%d" % newStateAvailable
...
def authenticateUser(self, publicKeyHex, privateKeyHex, ID, IDType, name):
    ...
    print
    "Nuki Pairing UUID handle created: %04x" % pairingHandle
    ...
    self._charWriteResponse = ""
    print
    "Requesting Nuki Public Key using command: %s" % publicKeyReq.show()
    ...
    print
    "Challenge received: %s" % commandParsed.nonce
    ...
    print
    "Authorization Authenticator sent: %s" % authAuthenticator.show()
    time.sleep(2)
    commandParsed = self.parser.parse(self._charWriteResponse)
    if self.parser.isNukiCommand(self._charWriteResponse) == False:
    ...
    print
    "Authorization Data sent: %s" % authData.show()
    time.sleep(2)
    commandParsed = self.parser.parse(self._charWriteResponse)
    if self.parser.isNukiCommand(self._charWriteResponse) == False:
    ...
    print
    "Authorization ID received: %s" % commandParsed.show()
    nonceNuki = commandParsed.nonce
    ...
    print
    "Authorization ID Confirmation sent: %s" % authIDConfirm.show()
    time.sleep(2)
    commandParsed = self.parser.parse(self._charWriteResponse)
    if self.parser.isNukiCommand(self._charWriteResponse) == False:
    ...
    print
    "STATUS received: %s" % commandParsed.status
...
def readLockState(self):
    ...
    stateReqEncrypted = nuki_messages.Nuki_EncryptedCommand(authID=self.config.get(self.macAddress, 'authorizationID'),
...
def lockAction(self, lockAction):
    ...
    print
    "Nuki CHALLENGE Request sent: %s" % challengeReq.show()
    time.sleep(2)
    commandParsed = self.parser.decrypt(self._charWriteResponse, self.config.get(self.macAddress, 'publicKeyNuki'),
                                        self.config.get(self.macAddress, 'privateKeyHex'))[8:]
    if self.parser.isNukiCommand(commandParsed) == False:
    ...
    print
    "Nuki Lock Action Request sent: %s" % lockActionReq.show()
    time.sleep(2)
    commandParsed = self.parser.decrypt(self._charWriteResponse, self.config.get(self.macAddress, 'publicKeyNuki'),
                                        self.config.get(self.macAddress, 'privateKeyHex'))[8:]
    if self.parser.isNukiCommand(commandParsed) == False:
...
def getLogEntriesCount(self, pinHex):
    ...
    self._charWriteResponse = ""
    print
    "Requesting CHALLENGE: %s" % challengeReqEncrypted.generate("HEX")
    ...
    print
    "Nuki Log Entries Request sent: %s" % logEntriesReq.show()
    time.sleep(2)
    commandParsed = self.parser.decrypt(self._charWriteResponse, self.config.get(self.macAddress, 'publicKeyNuki'),
                                        self.config.get(self.macAddress, 'privateKeyHex'))[8:]
    if self.parser.isNukiCommand(commandParsed) == False:
...
def getLogEntries(self, count, pinHex):
    ...
    print
    "Received %d messages" % len(messages)
    logMessages = []
    for message in messages:
        print
        "Decrypting message %s" % message
        try:
